...
const Party = require("../models/partySchemaModel");
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    res.send({
      ...
      emailSentmessage: "You Singup Successfully",
    });
  } catch (err) {
    console.log("Err", err);
...
const addHisab = async (req, res) => {
  ...
  try {
    ...
    let isPartyExist = false;
    console.log("partyId", req.body.partyId);
    console.log(id, "userId");
    let result = await PartyData.findOne({
      partyId: req.body.partyId,
      userId: id,
    });
    ...
    console.log(result, "result");
    if (!result) {
      const partyData = await PartyData.create({
        userId: id,
        partyId: req.body.partyId,
        id: new mongoose.mongo.ObjectId(),
        partyName: req.body.partyName,
      ...
      const partyData = await PartyData.findById(result._id);
      console.log(partyData, "partyData");
      partyData?.hisabKitab.push(hisab);
      partyData.save();
    }
    res.send({
      success: true,
      message: `You ${req.body.type} product added successfully`,
    });
  } catch (err) {
    console.log("Err", err);
    res.send({ success: false });
...
const upDateHisab = async (req, res) => {
  ...
  try {
    let result1 = await PartyData.findById(recordId);
    console.log("result1", result1);
    let index = result1?.hisabKitab.findIndex((x) => x.id == hisabId);
    console.log("index", index);
    console.log(
      " ...result1?.hisabKitab[index].paymentRcvd",
      result1?.hisabKitab[index],
    );
    console.log("result1?.hisabKitab", result1?.hisabKitab);

    let filterArray = result1?.hisabKitab.filter(
      (x) => x.id != result1?.hisabKitab[index].id,
    );
    result1?.hisabKitab[index].paymentRcvd.push({
      paymentRcvd:
        paymentRcvd > result1?.hisabKitab[index].remainingBal
          ? result1?.hisabKitab[index].remainingBal
          : paymentRcvd,
      remainingPayment:
        paymentRcvd > result1?.hisabKitab[index].remainingBal
          ? 0
          : result1?.hisabKitab[index].remainingBal - paymentRcvd,
      date: paymentDate,
    });
    console.log(
      "result1?.hisabKitab[index].paymentRcvd",
    ...
    console.log(
      "result1?.hisabKitab[index].paymentRcvd.paymentRcvd",
      result1?.hisabKitab[index].paymentRcvd.paymentRcvd,
    );
    ...
    let obj = {
      ...
      remainingBal:
        result1?.hisabKitab[index].remainingBal > 0
          ? result1?.hisabKitab[index].paymentRcvd[
              result1?.hisabKitab[index].paymentRcvd.length - 1
            ].remainingPayment
          : result1?.hisabKitab[index].totalPrice -
            result1?.hisabKitab[index].paymentRcvd,
    };
    ...
    let result = await PartyData.updateOne(
      {
        $and: [
          { _id: recordId },
...
const contactFormMessage = async (req, res) => {
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      ...
      res.send({
        success: false,
        message: "Message failed to sent Some error occured",
      });
    }
...
const removeOutward = async (req, res) => {
  ...
  try {
    ...
    console.log("rrrrrrr", partyData);
    partyData.hisabKitab = partyData.hisabKitab.filter(
      (rec) => rec.id != recId,
    );
...
const updateQuantity = async (req, res) => {
  ...
  try {
    let partyData = await PartyData.findById(mainId);
    if (!partyData) {
      return res.send({
        success: false,
...
const addParty = async (req, res) => {
  ...
  try {
    if (true) {
      const partyData = await Party.create({
        partyName: req.body.partyName,
        partyAddress: req.body.address,
        mobileNo: req.body.mobileNo,
        userId: req.body.userId,
      });
      console.log(partyData, "partyData");
    }
    res.send({ success: true, message: `Your Party added successfully` });
  } catch (err) {
    console.log("Err", err);
    res.send({ success: false });
  }
...
const addProductCategory = async (req, res) => {
  console.log("Req.body", req.body);
    try {
      const productCategoryData = await ProductCategory.create({
        categoryName: req.body.categoryName,
      });
    res.send({ success: true, message: `Your Product Category added successfully` });
  } catch (err) {
    console.log("Err", err);
    res.send({ success: false });
  }
